...
import { CURRENT_KLS_MURID, KLS_MURID, getAllKelasByMurid, getCurrentKelasMurid } from "@/components/Hooks/Murid";
...
export async function getServerSideProps(context) {
  ...
  if (authentications.rolesTrue) {
    if (role === "MURID", "ORANGTUA") {
      const list_kelas_murid = await getAllKelasByMurid(`${KLS_MURID}${username}`, token);
      const semester = await getListSemester();
      const current_kelas_murid = await getCurrentKelasMurid(`${CURRENT_KLS_MURID}${username}`, token)
      return {
        props: {
          role: role,
